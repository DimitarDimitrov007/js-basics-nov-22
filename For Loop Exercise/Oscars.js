function oscars(input){
    let name = input[0];
    let startingPoints = Number(input[1]);
    let judgesCount = Number(input[2]);
    let totalPoints = startingPoints;

    for(let i = 3; i < input.length; i+=2){
        let judgeName = input[i];
        let judgePoints = input[i +1];

         let totalPointsFromJudge =  (judgeName.length * judgePoints) / 2;
         totalPoints += totalPointsFromJudge;

        if(totalPoints > 1250.5){
            console.log(`Congratulations, ${name} got a nominee for leading role with ${totalPoints.toFixed(1)}!`);
            return;
        }

    }
  
        console.log(`Sorry, ${name} you need ${(1250.5 - totalPoints).toFixed(1)} more!`);
    }


oscars(["Zahari Baharov","205","4","Johnny Depp","45","Will Smith","29","Jet Lee","10","Matthew Mcconaughey","39"]);
oscars(["Sandra Bullock","340","5","Robert De Niro","50","Julia Roberts","40.5","Daniel Day-Lewis","39.4","Nicolas Cage","29.9","Stoyanka Mutafova",
"33"]);

